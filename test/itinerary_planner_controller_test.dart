...
class _FakeItineraryRepository extends ItineraryRepository {
  ...
  }) async => _buildFakePlan(destinations);
}
...
class _FakeSavedItineraryRepository extends SavedItineraryRepository {
  ...
  _FakeSavedItineraryRepository()
    : super(client: SupabaseClient('https://example.supabase.co', 'test-key'));

  int saveCalls = 0;
  int updateCalls = 0;
  final List<String> updatedIds = [];
  List<SavedItinerary> fetchValues = [];

  @override
  Future<List<SavedItinerary>> fetchAll() async => List.of(fetchValues);
...
void main() {
  ...
  test('saved itineraries reload for a different account', () async {
    final repository = _FakeSavedItineraryRepository();
    final store = SavedItinerariesStore(repository: repository);
    final userA = SavedItinerary(
      id: 'user-a-trip',
      plan: _buildFakePlan([_stopA]),
      savedAt: DateTime(2026),
    );
    final userB = SavedItinerary(
      id: 'user-b-trip',
      plan: _buildFakePlan([_stopB]),
      savedAt: DateTime(2026),
    );

    store.scopeToUser('user-a');
    repository.fetchValues = [userA];
    await store.ensureLoaded();
    expect(store.saved.single.id, userA.id);

    store.scopeToUser('user-b');
    expect(store.saved, isEmpty);
    repository.fetchValues = [userB];
    await store.ensureLoaded();
    expect(store.saved.single.id, userB.id);
  });
  ...
  test(
    ...
    () async {
      ...
      SavedItinerariesStore.instance = SavedItinerariesStore(
        repository: fakeSavedRepo,
      );
      ...
      expect(
        fakeSavedRepo.saveCalls,
        2,
        reason: 'must INSERT a second record, not update the first',
      );
  ...
  test(
    'editing a previously-saved itinerary via loadSavedItinerary still updates it in place',
    () async {
      final fakeSavedRepo = _FakeSavedItineraryRepository();
      SavedItinerariesStore.instance = SavedItinerariesStore(
        repository: fakeSavedRepo,
