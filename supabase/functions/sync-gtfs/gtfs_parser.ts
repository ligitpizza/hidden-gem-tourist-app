...
const rows = (files: Record<string, Uint8Array>, name: string) =>
  files[name]
    ? parse(decoder.decode(files[name]), {
      columns: true,
...
const defaultMaxStopTimesBytes = 8 * 1024 * 1024;
const importedGtfsFiles = new Set([
  "agency.txt",
  "routes.txt",
  "stops.txt",
  "trips.txt",
...
export function parseGtfs(
  ...
  const maxStopTimesBytes = options.maxStopTimesBytes ??
    defaultMaxStopTimesBytes;
  const files = unzipSync(zip, {
    filter(file) {
      if (!importedGtfsFiles.has(file.name)) return false;
      return file.name !== "stop_times.txt" ||
        file.originalSize <= maxStopTimesBytes;
    },
  });
  const stopRoutesOmitted = !files["stop_times.txt"];
  const agencies = rows(files, "agency.txt").map((
    a: Record<string, string>,
  ) => ({
    id: `${feedId}:${a.agency_id || "default"}`,
    feed_id: feedId,
  ...
  const stops = rows(files, "stops.txt").filter((s: Record<string, string>) =>
    ...
    stop_id: s.stop_id,
    name: s.stop_name,
    address: s.stop_desc?.trim() || `${s.stop_name}, Malaysia`,
  ...
  const routes = routesRaw.map((r) => ({
    ...
    mode: transportMode(r),
  }));
  const tripRoutes = new Map(
    (rows(files, "trips.txt") as Record<string, string>[]).map(
      (t) => [t.trip_id, t.route_id],
    ),
  );
  ...
  const stop_routes = [...pairs].map((pair) => {
    const [stop_id, route_id] = pair.split("\u0000");
    return { stop_id, route_id };
  });
  return { agencies, stops, routes, stop_routes, stopRoutesOmitted };
}
